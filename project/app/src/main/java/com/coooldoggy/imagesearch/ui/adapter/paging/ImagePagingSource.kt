...
class ImagePagingSource(val imageSearchService: ImageSearchService, val query: String) : PagingSource<Int, Documents>(){
    ...
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, Documents> {
        try {
            ...
            val response = imageSearchService.queryImage(key = ImageSearchApplication.getContext()?.getString(R.string.kakao_app_key) ?: "", query= query, page = nextPageNumber)
            return LoadResult.Page(
                data = response.body()?.documents ?: emptyList(),
                prevKey = null,
                nextKey = if (response.body()?.meta?.isEnd == true) null else params.key?.plus(1)
            )
        }catch (e: Exception){
           e.printStackTrace()
        }
